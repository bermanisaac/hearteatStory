...
-- Only GET, POST requests for now.
data HTTPRequest =
    GetRequest { path :: String }
  | PostRequest {actionURI :: String, bodyLength :: Int}
    deriving (Show)
...
statusOk, statusCreated, statusBadRequest, statusNotFound :: B.ByteString
statusOk         = "200 OK"
...
censor f = WriterT . fmap (mapSnd f) . runWriterT where
  mapSnd f (a, b) = (a, f b)
...
instance Functor m => Functor (WriterT w m) where
    ...
    fmap f = WriterT . fmap (mapFst f) . runWriterT where
      mapFst f (a,b) = (f a, b)
...
instance (Monoid w, Applicative m) => Applicative (WriterT w m) where
    ...
    writerTFunc <*> writerTArg =
        -- I based this off of the implementation in Control.Monad.Trans.Writer, but only for the sake of cleaner code than my first solution
           WriterT $ liftA2 combinePairs (runWriterT writerTFunc) (runWriterT writerTArg) where
             combinePairs (f, w) (a, w2) = (f a, w <> w2)
        ---------------FIRST VERSION----------------------------------
        -- WriterT $ liftA2 (,) (fM <*> argM) fullMessage where
        --   argPair = runWriterT writerTArg
        --   fPair = runWriterT writerTFunc
        --   fM = fst <$> fPair
        --   argM = fst <$> argPair
        --   fW = snd <$> fPair
        --   argW = snd <$> argPair
        --   fullMessage = liftA2 (<>) fW argW

instance (Monoid w, Monad m) => Monad (WriterT w m) where
    (>>=) :: (Monoid w, Monad m) => WriterT w m a -> (a -> WriterT w m b) -> WriterT w m b
    writerT >>= f = WriterT $ do
      (a, w) <- runWriterT writerT
      (a', w') <- runWriterT $ f a
      pure (a', w <> w')
    ...
    case maybeReqLines of
        ...
        Just reqLines ->
            case maybeParseRequestLines reqLines of
                ...
                Just request -> do
                        tell [log request]
                        response <- lift $ createResponse request connHandle
                        doResponse response
    where
        ...
        log (GetRequest path) = "Requested " ++ path
        log (PostRequest action _) = "Form action " ++ action
...
maybeParseRequestLines request@(firstLine:_) =
    case words firstLine of
        [ "GET", path, "HTTP/1.1" ]       -> Just $ GetRequest path
        [ "POST", actionURI, "HTTP/1.1" ] -> Just $ PostRequest actionURI (getContentLength request)
        _                                 -> Nothing
    where
      getBody = unlines . tail . dropWhile (/= "")

      getContentLength :: [String] -> Int
      getContentLength (line:rest) =  case stripPrefix "Content-Length: " line of
        Just num -> read num
        Nothing -> getContentLength rest
      getContentLength [] = 0
